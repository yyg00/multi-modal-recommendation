...
from utils.data_loader import load_item_image, load_item_name, load_item_address, load_data, load_item_data, SequenceDataset, ItemDataset
...
def evaluate(model, eval_seq_dataloader, eval_item_dataloader, device, Ks, logging,tb_logger,step):
    ...
    with torch.no_grad():
        for i, batch in enumerate(eval_item_dataloader):
            ...
            item_imgs = batch["item_imgs"].to(device) # images
            # print(torch.isnan(item_inputs).any())
            # print(torch.isnan(item_masks).any())
            # print(torch.isnan(item_imgs).any())
            _,item_emb = model(item_inputs, item_masks, item_imgs)
        ...
        for i, batch in enumerate(eval_seq_dataloader):
            ...
            seq_imgs = batch["seq_imgs"].to(device) # images
            batch_target = batch["target_list"]
            _,seq_emb = model(seq_inputs, seq_masks, seq_imgs)
...
def main():
    ...
    image_file = os.path.join(data_dir, 'item_photos.npy') # images path
    ...
    if opt.data_name == 'beauty' or opt.data_name == 'sports' or opt.data_name == 'toys':
        ...
        item_img = load_item_image(image_file) # load images

    item_len = len(item_desc)
    logging.info(f"item len: {item_len}")
    eval_data = load_data(eval_file,item_desc,item_img)
    logging.info(f"dev len: {len(eval_data)}")
    item_data = load_item_data(item_desc, item_img)

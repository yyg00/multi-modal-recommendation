...
class DR4RecModel(nn.Module):
    ...
    def forward(
        ...
        # print(q_reps.isnan().any(), p_reps.isnan().any())
        if q_reps.isnan().any() or p_reps.isnan().any():
            return
    ...
    def encode(self, items, model, head, passage=False):
        if items is None:
            return None, None
        input_ids = items[0]
        attention_mask = items[1]
        image_embedding = items[2] # split for image embedding
        hidden, reps = model(input_ids, attention_mask, image_embedding)
